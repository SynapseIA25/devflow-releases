...
const TOOLS = [
  ...
  { name: "devflow_suggest_test_strategy", description: "Detecta el stack del proyecto actual (lenguaje, framework de UI, motor de render) y sugiere una estrategia de testing: backend de automatización, MCP servers recomendados y patrones de caso de prueba. Usa una tabla conocida para stacks comunes (web/Chromium, mobile, CLI) o delega al agente QA si el stack no matchea ninguno. Persiste la sugerencia para que devflow_insert_test_case pueda usarla después.", inputSchema: { type: "object", properties: {} } },
  { name: "devflow_insert_test_case", description: "Genera e inserta un caso de test real para un patrón sugerido por devflow_suggest_test_strategy (llamalo primero). Busca un archivo de test existente del proyecto como referencia de estilo y lo inserta ahí (o crea uno nuevo si no hay ninguno). Para patrones simples (smoke/cli-smoke) el cuerpo queda con un TODO explícito; para el resto lo completa el agente QA — tratalo como un borrador a revisar, no como un resultado garantizado.", inputSchema: { type: "object", properties: { patternId: { type: "string", description: "id de un patrón devuelto por devflow_suggest_test_strategy (ej. 'smoke', 'form-validation')" }, targetHint: { type: "string", description: "qué testear específicamente (módulo, componente, endpoint)" } }, required: ["patternId", "targetHint"] } },
];
